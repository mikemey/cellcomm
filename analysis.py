...
with open('logs/first.txt') as f:
    xs = []
    all_loss_graphs = [[], [], [], []]
    for line in f.readlines():
        def interested_cols(token):
            return len(token) > 0 and not re.search('[a-zA-Z]', token)
        ...
        tokens = map(clean, filter(interested_cols, line.split(' ')))
